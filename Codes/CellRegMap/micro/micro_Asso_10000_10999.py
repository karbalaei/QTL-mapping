...
sample_mapping = pd.read_csv('micro_sample_mapping_file.csv', dtype={'genotype_individual_id': str, 'phenotype_sample_id': str})
...
# Cordinates (PCAs)
E_file = 'micro_CellRegMap_PCAs.txt'
...
#Annotation file
annotation_file = 'micro_annotation.txt'
...
W= pd.read_csv('micro_CellRegMap_covariate.txt', index_col=0, sep='	')
...
def CellRegMap_interaction(gene,anno_df, G, w, W, E , hK , results):
    ...
    df.to_csv('/data/fgoes1/RK/CellRegMap/micro/'+ gene + '_micro_Association.csv',sep=',',index=False)
