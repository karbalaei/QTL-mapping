...
sample_mapping = pd.read_csv('oligo_sample_mapping_file.csv', dtype={'genotype_individual_id': str, 'phenotype_sample_id': str})
...
# Cordinates (PCAs)
E_file = 'oligo_CellRegMap_PCAs.txt'
...
#Annotation file
annotation_file = 'oligo_annotation.txt'
...
W= pd.read_csv('oligo_CellRegMap_covariate.txt', index_col=0, sep='	')
...
def CellRegMap_interaction(gene,anno_df, G, w, W, E , hK , results):
    ...
    df.to_csv('/data/fgoes1/RK/CellRegMap/oligo/'+ gene + '_oligo_Association.csv',sep=',',index=False)
